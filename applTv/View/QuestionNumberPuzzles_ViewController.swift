...
class QuestionNumberPuzzles_ViewController: UIViewController {
    var ourTimer = Timer()
    ...
    func getHomeNimeManga() {
        APIService.shared.GetAllQuestionNumberPuzzles() { [self] (response, error) in
    ...
    func loadQuestion(){
        ...
        let numberRandom = Int.random(in: 20..<100)
        let numberTru = Int.random(in: 0..<10)
        
            explanation = listData[indexQuestion].explanation
            answer = listData[indexQuestion].answer_true_text
            let listAnswerRandum = [numberRandom,
                                    numberRandom - numberTru,
                                    Int(listData[indexQuestion].answer_true_text ?? "") ?? 0,
                                    numberRandom - numberTru + 1]
            
            let randomAnswer = Int.random(in: 0..<4)
            orderQuestion.text = "Question " + String(indexQuestion) + "/279"
            let url = URL(string: listData[indexQuestion].question_image ?? "")
            imageQuestion.kf.setImage(with: url)
            if randomAnswer == 0{
                outletBTA.setTitle(String(listAnswerRandum[randomAnswer]), for: .normal)
                outletBTB.setTitle(String(listAnswerRandum[1]), for: .normal)
                outletBTC.setTitle(String(listAnswerRandum[2]), for: .normal)
                outletBTD.setTitle(String(listAnswerRandum[3]), for: .normal)
            }else if randomAnswer == 1{
                outletBTA.setTitle(String(listAnswerRandum[randomAnswer]), for: .normal)
                outletBTB.setTitle(String(listAnswerRandum[0]), for: .normal)
                outletBTC.setTitle(String(listAnswerRandum[2]), for: .normal)
                outletBTD.setTitle(String(listAnswerRandum[3]), for: .normal)
            }else if randomAnswer == 2{
                outletBTA.setTitle(String(listAnswerRandum[randomAnswer]), for: .normal)
                outletBTB.setTitle(String(listAnswerRandum[1]), for: .normal)
                outletBTC.setTitle(String(listAnswerRandum[0]), for: .normal)
                outletBTD.setTitle(String(listAnswerRandum[3]), for: .normal)
            }else if randomAnswer == 3{
                outletBTA.setTitle(String(listAnswerRandum[randomAnswer]), for: .normal)
                outletBTB.setTitle(String(listAnswerRandum[1]), for: .normal)
                outletBTC.setTitle(String(listAnswerRandum[2]), for: .normal)
                outletBTD.setTitle(String(listAnswerRandum[0]), for: .normal)
            }
//        }
